...
// EN Methods for creating the FluidSp class.
// PT Métodos de criação da classe FluidSp.

/**
 * EN Starts the build chain for FluidSp from a base TextUnit (Sp).
 * Usage example: `16.sp.fluidSp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para FluidSp a partir de um TextUnit (Sp) base.
 ...
 * Usage example: `16.fluidSp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para FluidSp a partir de um Int base (tratado como sp).
 * Exemplo de uso: `16.fluidSp().screen(...)`.
 */
@Composable
fun Number.fluidSp(): FluidSp = FluidSp(this)
 ...
 */
@Stable
class FluidSp private constructor(
    ...
    fun ignoreMultiWindows(ignore: Boolean = true): FluidSp {
        return FluidSp(initialBaseValue, defaultFontScale, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
    ...
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): FluidSp {
        return FluidSp(initialBaseValue, defaultFontScale, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(
        ...
        return FluidSp(initialBaseValue, defaultFontScale, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolve(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicFluidSp(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Same as [resolve] but returns density-scaled pixels ([toDynamicFluidPx]).
     * PT Igual a [resolve], mas retorna pixels já escalados pela densidade ([toDynamicFluidPx]).
     */
    @Composable
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicFluidPx(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Like [resolve] but forces `fontScale = false` (fixed Sp, same idea as `sem` / `fwem` accessors).
     * PT Como [resolve], mas força `fontScale = false` (Sp fixo, mesmo propósito dos acessores `sem` / `fwem`).
     */
    @Composable
    private fun resolveNoFontScale(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicFluidSp(finalQualifier, fontScale = false, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolveNoFontScalePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicFluidPx(finalQualifier, fontScale = false, inverter = foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows = ignoreMultiWindows, applyAspectRatio = applyAspectRatio, customSensitivityK = customSensitivityK)
    }
     ...
     */
    @get:Composable
    val fssp: TextUnit get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val fhsp: TextUnit get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val fwsp: TextUnit get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val fhem: TextUnit get() = resolveNoFontScale(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val fwem: TextUnit get() = resolveNoFontScale(DpQualifier.WIDTH)
