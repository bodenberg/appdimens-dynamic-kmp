...
// EN Methods for creating the PerimeterSp class.
// PT Métodos de criação da classe PerimeterSp.

/**
 * EN Starts the build chain for PerimeterSp from a base TextUnit (Sp).
 * Usage example: `16.sp.perimeterSp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para PerimeterSp a partir de um TextUnit (Sp) base.
 ...
 * Usage example: `16.perimeterSp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para PerimeterSp a partir de um Int base (tratado como sp).
 * Exemplo de uso: `16.perimeterSp().screen(...)`.
 */
@Composable
fun Number.perimeterSp(): PerimeterSp = PerimeterSp(this)
 ...
 */
@Stable
class PerimeterSp private constructor(
    ...
    fun ignoreMultiWindows(ignore: Boolean = true): PerimeterSp {
        return PerimeterSp(initialBaseValue, defaultFontScale, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
    ...
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): PerimeterSp {
        return PerimeterSp(initialBaseValue, defaultFontScale, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(
        ...
        return PerimeterSp(initialBaseValue, defaultFontScale, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolve(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicPerimeterSp(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Same as [resolve] but returns density-scaled pixels ([toDynamicPerimeterPx]).
     * PT Igual a [resolve], mas retorna pixels já escalados pela densidade ([toDynamicPerimeterPx]).
     */
    @Composable
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicPerimeterPx(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Like [resolve] but forces `fontScale = false` (fixed Sp, same idea as `sem` / `prwem` accessors).
     * PT Como [resolve], mas força `fontScale = false` (Sp fixo, mesmo propósito dos acessores `sem` / `prwem`).
     */
    @Composable
    private fun resolveNoFontScale(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicPerimeterSp(finalQualifier, fontScale = false, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolveNoFontScalePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicPerimeterPx(finalQualifier, fontScale = false, inverter = foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows = ignoreMultiWindows, applyAspectRatio = applyAspectRatio, customSensitivityK = customSensitivityK)
    }
     ...
     */
    @get:Composable
    val prssp: TextUnit get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val prhsp: TextUnit get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val prwsp: TextUnit get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val prhem: TextUnit get() = resolveNoFontScale(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val prwem: TextUnit get() = resolveNoFontScale(DpQualifier.WIDTH)
