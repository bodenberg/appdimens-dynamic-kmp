 ...
 * Used by the DimenPercentScaled class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenPercentScaled para definir valores específicos para condições de tela.
 *
...
// EN Methods for creating the DimenPercentScaled class.
// PT Métodos de criação da classe DimenPercentScaled.

/**
 * EN Starts the build chain for the custom dimension DimenPercentScaled from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPercentScaled a partir de um Dp base.
 */
@Composable
fun Dp.percentDp(): DimenPercentScaled = DimenPercentScaled(this@percentDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenPercentScaled from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenPercentScaled a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.percentDp(): DimenPercentScaled = this.toFloat().dp.percentDp()
 ...
 */
@Stable
class DimenPercentScaled private constructor(
     ...
     */
    fun ignoreMultiWindows(ignore: Boolean = true): DimenPercentScaled {
        return DimenPercentScaled(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenPercentScaled {
        return DimenPercentScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(
        ...
        return DimenPercentScaled(initialBaseDp, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenPercentScaled {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
    ...
    private fun resolve(qualifier: DpQualifier): Dp {
        ...
        return baseDp.toDynamicPercentDp(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        val finalQualifier = foundEntry?.finalQualifierResolver ?: qualifier
        val baseDp = dpToUse.value
        return baseDp.toDynamicPercentPx(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
