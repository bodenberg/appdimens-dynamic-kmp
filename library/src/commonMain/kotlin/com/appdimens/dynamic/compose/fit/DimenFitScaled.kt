...
/**
 * EN Starts the build chain for the custom dimension DimenFitScaled from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenFitScaled a partir de um Dp base.
 */
@Composable
fun Dp.fitDp(): DimenFitScaled = DimenFitScaled(this@fitDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenFitScaled from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenFitScaled a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.fitDp(): DimenFitScaled = this.toFloat().dp.fitDp()
...
class DimenFitScaled private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenFitScaled {
        return DimenFitScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenFitScaled {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
    ...
    private fun resolve(qualifier: DpQualifier): Dp {
        ...
        return baseDp.toDynamicFitDp(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        val finalQualifier = foundEntry?.finalQualifierResolver ?: qualifier
        val baseDp = dpToUse.value
        return baseDp.toDynamicFitPx(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
