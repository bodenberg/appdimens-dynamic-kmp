...
/**
 * EN Starts the build chain for the custom dimension DimenScaled from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenScaled a partir de um Dp base.
 */
@Composable
 ...
 */
@Composable
fun Number.scaledDp(): DimenScaled = this.toFloat().dp.scaledDp()
...
class DimenScaled private constructor(
    ...
    private fun resolve(qualifier: DpQualifier): Dp {
        ...
        return baseDp.toDynamicScaledDp(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        val finalQualifier = foundEntry?.finalQualifierResolver ?: qualifier
        val baseDp = dpToUse.value
        return baseDp.toDynamicScaledPx(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
