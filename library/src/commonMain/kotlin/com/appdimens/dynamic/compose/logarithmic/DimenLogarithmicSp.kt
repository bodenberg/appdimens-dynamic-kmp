 ...
 * Usage example: `16.lognem`.
 *
 * PT
 * Extensão para Int com dimensionamento dinâmico baseado na **Smallest Width (swDP)**,
 * mas sem respeitar a escala de fonte do sistema.
 * Exemplo de uso: `16.lognem`.
 */
@get:Composable
val Number.lognem: TextUnit get() = this.toDynamicLogarithmicSp(DpQualifier.SMALL_WIDTH, fontScale = false)
/** a variant explicitly with applyAspectRatio */
@get:Composable
val Number.nema: TextUnit get() = this.toDynamicLogarithmicSp(DpQualifier.SMALL_WIDTH, fontScale = false, applyAspectRatio = true)
/** i variant explicitly with ignoreMultiWindows */
@get:Composable
val Number.nemi: TextUnit get() = this.toDynamicLogarithmicSp(DpQualifier.SMALL_WIDTH, fontScale = false, ignoreMultiWindows = true)
/** ia variant explicitly with both */
@get:Composable
val Number.nemia: TextUnit get() = this.toDynamicLogarithmicSp(DpQualifier.SMALL_WIDTH, fontScale = false, ignoreMultiWindows = true, applyAspectRatio = true)

@get:Composable
val Number.nemPx: Float get() = this.toDynamicLogarithmicPx(DpQualifier.SMALL_WIDTH, fontScale = false)
@get:Composable
val Number.nemPxa: Float get() = this.toDynamicLogarithmicPx(DpQualifier.SMALL_WIDTH, fontScale = false, applyAspectRatio = true)
@get:Composable
val Number.nemPxi: Float get() = this.toDynamicLogarithmicPx(DpQualifier.SMALL_WIDTH, fontScale = false, ignoreMultiWindows = true)
@get:Composable
val Number.nemPxia: Float get() = this.toDynamicLogarithmicPx(DpQualifier.SMALL_WIDTH, fontScale = false, ignoreMultiWindows = true, applyAspectRatio = true)
 ...
 * 3. If [fontScale] is `false` (e.g. via [.lognem]), the system font scale is stripped.
 ...
 * 3. Se [fontScale] for `false` (ex: via [.lognem]), a escala de fonte do sistema é removida.
...
): Float = calculateLogarithmicDpCompose(baseValue, metrics, qualifier, inverter, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
...
internal fun rememberLogarithmicSp(
    ...
    DimenCache.getOrPut(cacheKey, ctx.cachePersistence) {
        val raw = calculateLogarithmicSpValueCompose(baseValue, qualifier, inverter, ignoreMultiWindows, applyAspectRatio, customSensitivityK, metrics)
...
internal fun rememberLogarithmicSpPx(
    ...
    DimenCache.getOrPut(cacheKey, ctx.cachePersistence) {
        val scaledVal = calculateLogarithmicSpValueCompose(baseValue, qualifier, inverter, ignoreMultiWindows, applyAspectRatio, customSensitivityK, metrics)
