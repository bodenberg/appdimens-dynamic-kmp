...
// EN Methods for creating the LogarithmicSp class.
// PT Métodos de criação da classe LogarithmicSp.

/**
 * EN Starts the build chain for LogarithmicSp from a base TextUnit (Sp).
 * Usage example: `16.sp.logarithmicSp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para LogarithmicSp a partir de um TextUnit (Sp) base.
 ...
 * Usage example: `16.logarithmicSp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para LogarithmicSp a partir de um Int base (tratado como sp).
 * Exemplo de uso: `16.logarithmicSp().screen(...)`.
 */
@Composable
fun Number.logarithmicSp(): LogarithmicSp = LogarithmicSp(this)
 ...
 */
@Stable
class LogarithmicSp private constructor(
     ...
     */
    fun ignoreMultiWindows(ignore: Boolean = true): LogarithmicSp {
        return LogarithmicSp(initialBaseValue, defaultFontScale, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
    ...
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): LogarithmicSp {
        return LogarithmicSp(initialBaseValue, defaultFontScale, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(
        ...
        return LogarithmicSp(initialBaseValue, defaultFontScale, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolve(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicLogarithmicSp(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Same as [resolve] but returns density-scaled pixels ([toDynamicLogarithmicPx]).
     * PT Igual a [resolve], mas retorna pixels já escalados pela densidade ([toDynamicLogarithmicPx]).
     */
    @Composable
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicLogarithmicPx(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Like [resolve] but forces `fontScale = false` (fixed Sp, same idea as `sem` / `logwem` accessors).
     * PT Como [resolve], mas força `fontScale = false` (Sp fixo, mesmo propósito dos acessores `sem` / `logwem`).
     */
    @Composable
    private fun resolveNoFontScale(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicLogarithmicSp(finalQualifier, fontScale = false, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolveNoFontScalePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicLogarithmicPx(finalQualifier, fontScale = false, inverter = foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows = ignoreMultiWindows, applyAspectRatio = applyAspectRatio, customSensitivityK = customSensitivityK)
    }
     ...
     */
    @get:Composable
    val logssp: TextUnit get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val loghsp: TextUnit get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val logwsp: TextUnit get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val loghem: TextUnit get() = resolveNoFontScale(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val logwem: TextUnit get() = resolveNoFontScale(DpQualifier.WIDTH)
