 ...
 * Used by the DimenDensityScaled class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenDensityScaled para definir valores específicos para condições de tela.
 *
...
// EN Methods for creating the DimenDensityScaled class.
// PT Métodos de criação da classe DimenDensityScaled.

/**
 * EN Starts the build chain for the custom dimension DimenDensityScaled from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDensityScaled a partir de um Dp base.
 */
@Composable
fun Dp.densityDp(): DimenDensityScaled = DimenDensityScaled(this@densityDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenDensityScaled from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenDensityScaled a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.densityDp(): DimenDensityScaled = this.toFloat().dp.densityDp()
 ...
 */
@Stable
class DimenDensityScaled private constructor(
     ...
     */
    fun ignoreMultiWindows(ignore: Boolean = true): DimenDensityScaled {
        return DimenDensityScaled(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenDensityScaled {
        return DimenDensityScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(
        ...
        return DimenDensityScaled(initialBaseDp, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenDensityScaled {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
    ...
    private fun resolve(qualifier: DpQualifier): Dp {
        ...
        return baseDp.toDynamicDensityDp(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        val finalQualifier = foundEntry?.finalQualifierResolver ?: qualifier
        val baseDp = dpToUse.value
        return baseDp.toDynamicDensityPx(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
