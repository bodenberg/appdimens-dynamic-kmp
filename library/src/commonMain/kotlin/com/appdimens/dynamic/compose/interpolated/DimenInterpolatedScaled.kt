 ...
 * Used by the DimenInterpolatedScaled class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenInterpolatedScaled para definir valores específicos para condições de tela.
 *
...
// EN Methods for creating the DimenInterpolatedScaled class.
// PT Métodos de criação da classe DimenInterpolatedScaled.

/**
 * EN Starts the build chain for the custom dimension DimenInterpolatedScaled from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenInterpolatedScaled a partir de um Dp base.
 */
@Composable
fun Dp.interpolatedDp(): DimenInterpolatedScaled = DimenInterpolatedScaled(this@interpolatedDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenInterpolatedScaled from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenInterpolatedScaled a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.interpolatedDp(): DimenInterpolatedScaled = this.toFloat().dp.interpolatedDp()
 ...
 */
@Stable
class DimenInterpolatedScaled private constructor(
     ...
     */
    fun ignoreMultiWindows(ignore: Boolean = true): DimenInterpolatedScaled {
        return DimenInterpolatedScaled(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenInterpolatedScaled {
        return DimenInterpolatedScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    private fun reorderEntries(newEntry: CustomDpEntry): List<CustomDpEntry> {
        return (sortedCustomEntries + newEntry).sortedWith(
            compareBy<CustomDpEntry> { it.priority } // EN Priority 1 (most specific) first. / PT Prioridade 1 (mais específico) primeiro.
                .thenByDescending { it.dpQualifierEntry?.value?.toFloat() ?: 0f } // EN Higher value qualifier first. / PT Qualificador de valor maior primeiro.
        )
    }
    ...
    fun screen(
        ...
        return DimenInterpolatedScaled(initialBaseDp, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        val entry = CustomDpEntry(
            ...
            dpQualifierEntry = DpQualifierEntry(qualifierType, qualifierValue),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
            finalQualifierResolver = finalQualifierResolver,
            priority = 1,
            inverter = inverter
        )
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenInterpolatedScaled {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
    ...
    private fun resolve(qualifier: DpQualifier): Dp {
        ...
        return baseDp.toDynamicInterpolatedDp(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        val finalQualifier = foundEntry?.finalQualifierResolver ?: qualifier
        val baseDp = dpToUse.value
        return baseDp.toDynamicInterpolatedPx(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
