...
/**
 * EN Starts the build chain for the custom dimension DimenAutoScaled from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenAutoScaled a partir de um Dp base.
 */
@Composable
fun Dp.autoDp(): DimenAutoScaled = DimenAutoScaled(this@autoDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenAutoScaled from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenAutoScaled a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.autoDp(): DimenAutoScaled = this.toFloat().dp.autoDp()
...
class DimenAutoScaled private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenAutoScaled {
        return DimenAutoScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenAutoScaled {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
