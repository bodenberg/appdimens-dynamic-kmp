...
/**
 * EN Starts the build chain for DensitySp from a base TextUnit (Sp).
 * Usage example: `16.sp.densitySp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para DensitySp a partir de um TextUnit (Sp) base.
...
class DensitySp private constructor(
    ...
    private fun resolve(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicDensitySp(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Same as [resolve] but returns density-scaled pixels ([toDynamicDensityPx]).
     * PT Igual a [resolve], mas retorna pixels já escalados pela densidade ([toDynamicDensityPx]).
     */
    @Composable
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicDensityPx(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Like [resolve] but forces `fontScale = false` (fixed Sp, same idea as `sem` / `dwem` accessors).
     * PT Como [resolve], mas força `fontScale = false` (Sp fixo, mesmo propósito dos acessores `sem` / `dwem`).
     */
    @Composable
    private fun resolveNoFontScale(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicDensitySp(finalQualifier, fontScale = false, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolveNoFontScalePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicDensityPx(finalQualifier, fontScale = false, inverter = foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows = ignoreMultiWindows, applyAspectRatio = applyAspectRatio, customSensitivityK = customSensitivityK)
    }
     ...
     */
    @get:Composable
    val dssp: TextUnit get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val dhsp: TextUnit get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val dwsp: TextUnit get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val dhem: TextUnit get() = resolveNoFontScale(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val dwem: TextUnit get() = resolveNoFontScale(DpQualifier.WIDTH)
