...
// EN Methods for creating the DiagonalSp class.
// PT Métodos de criação da classe DiagonalSp.

/**
 * EN Starts the build chain for DiagonalSp from a base TextUnit (Sp).
 * Usage example: `16.sp.diagonalSp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para DiagonalSp a partir de um TextUnit (Sp) base.
 ...
 * Usage example: `16.diagonalSp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para DiagonalSp a partir de um Int base (tratado como sp).
 * Exemplo de uso: `16.diagonalSp().screen(...)`.
 */
@Composable
fun Number.diagonalSp(): DiagonalSp = DiagonalSp(this)
 ...
 */
@Stable
class DiagonalSp private constructor(
     ...
     */
    fun ignoreMultiWindows(ignore: Boolean = true): DiagonalSp {
        return DiagonalSp(initialBaseValue, defaultFontScale, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DiagonalSp {
        return DiagonalSp(initialBaseValue, defaultFontScale, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(
        ...
        return DiagonalSp(initialBaseValue, defaultFontScale, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolve(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicDiagonalSp(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Same as [resolve] but returns density-scaled pixels ([toDynamicDiagonalPx]).
     * PT Igual a [resolve], mas retorna pixels já escalados pela densidade ([toDynamicDiagonalPx]).
     */
    @Composable
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicDiagonalPx(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Like [resolve] but forces `fontScale = false` (fixed Sp, same idea as `sem` / `dgwem` accessors).
     * PT Como [resolve], mas força `fontScale = false` (Sp fixo, mesmo propósito dos acessores `sem` / `dgwem`).
     */
    @Composable
    private fun resolveNoFontScale(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicDiagonalSp(finalQualifier, fontScale = false, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolveNoFontScalePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicDiagonalPx(finalQualifier, fontScale = false, inverter = foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows = ignoreMultiWindows, applyAspectRatio = applyAspectRatio, customSensitivityK = customSensitivityK)
    }
     ...
     */
    @get:Composable
    val dgssp: TextUnit get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val dghsp: TextUnit get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val dgwsp: TextUnit get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val dghem: TextUnit get() = resolveNoFontScale(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val dgwem: TextUnit get() = resolveNoFontScale(DpQualifier.WIDTH)
