...
/**
 * EN Starts the build chain for the custom dimension DimenFillScaled from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenFillScaled a partir de um Dp base.
 */
@Composable
fun Dp.fillDp(): DimenFillScaled = DimenFillScaled(this@fillDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenFillScaled from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenFillScaled a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.fillDp(): DimenFillScaled = this.toFloat().dp.fillDp()
...
class DimenFillScaled private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenFillScaled {
        return DimenFillScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenFillScaled {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
    ...
    private fun resolve(qualifier: DpQualifier): Dp {
        ...
        return baseDp.toDynamicFillDp(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        val finalQualifier = foundEntry?.finalQualifierResolver ?: qualifier
        val baseDp = dpToUse.value
        return baseDp.toDynamicFillPx(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
