...
// EN Methods for creating the PercentSp class.
// PT Métodos de criação da classe PercentSp.

/**
 * EN Starts the build chain for PercentSp from a base TextUnit (Sp).
 * Usage example: `16.sp.percentSp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para PercentSp a partir de um TextUnit (Sp) base.
 ...
 * Usage example: `16.percentSp().screen(...)`.
 *
 * PT Inicia a cadeia de construção para PercentSp a partir de um Int base (tratado como sp).
 * Exemplo de uso: `16.percentSp().screen(...)`.
 */
@Composable
fun Number.percentSp(): PercentSp = PercentSp(this)
 ...
 */
@Stable
class PercentSp private constructor(
     ...
     */
    fun ignoreMultiWindows(ignore: Boolean = true): PercentSp {
        return PercentSp(initialBaseValue, defaultFontScale, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): PercentSp {
        return PercentSp(initialBaseValue, defaultFontScale, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(
        ...
        return PercentSp(initialBaseValue, defaultFontScale, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolve(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicPercentSp(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Same as [resolve] but returns density-scaled pixels ([toDynamicPercentPx]).
     * PT Igual a [resolve], mas retorna pixels já escalados pela densidade ([toDynamicPercentPx]).
     */
    @Composable
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicPercentPx(finalQualifier, finalFontScale, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }

    /**
     * EN Like [resolve] but forces `fontScale = false` (fixed Sp, same idea as `sem` / `pwem` accessors).
     * PT Como [resolve], mas força `fontScale = false` (Sp fixo, mesmo propósito dos acessores `sem` / `pwem`).
     */
    @Composable
    private fun resolveNoFontScale(qualifier: DpQualifier): TextUnit {
        ...
        return valueToUse.toDynamicPercentSp(finalQualifier, fontScale = false, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolveNoFontScalePx(qualifier: DpQualifier): Float {
        ...
        return valueToUse.toDynamicPercentPx(finalQualifier, fontScale = false, inverter = foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows = ignoreMultiWindows, applyAspectRatio = applyAspectRatio, customSensitivityK = customSensitivityK)
    }
     ...
     */
    @get:Composable
    val pssp: TextUnit get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val phsp: TextUnit get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val pwsp: TextUnit get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val phem: TextUnit get() = resolveNoFontScale(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val pwem: TextUnit get() = resolveNoFontScale(DpQualifier.WIDTH)
