 ...
 * Used by the DimenPerimeterScaled class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenPerimeterScaled para definir valores específicos para condições de tela.
 *
...
// EN Methods for creating the DimenPerimeterScaled class.
// PT Métodos de criação da classe DimenPerimeterScaled.

/**
 * EN Starts the build chain for the custom dimension DimenPerimeterScaled from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPerimeterScaled a partir de um Dp base.
 */
@Composable
fun Dp.perimeterDp(): DimenPerimeterScaled = DimenPerimeterScaled(this@perimeterDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenPerimeterScaled from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenPerimeterScaled a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.perimeterDp(): DimenPerimeterScaled = this.toFloat().dp.perimeterDp()
 ...
 */
@Stable
class DimenPerimeterScaled private constructor(
     ...
     */
    fun ignoreMultiWindows(ignore: Boolean = true): DimenPerimeterScaled {
        return DimenPerimeterScaled(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenPerimeterScaled {
        return DimenPerimeterScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(
        ...
        return DimenPerimeterScaled(initialBaseDp, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenPerimeterScaled {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
    ...
    private fun resolve(qualifier: DpQualifier): Dp {
        ...
        return baseDp.toDynamicPerimeterDp(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    private fun resolvePx(qualifier: DpQualifier): Float {
        ...
        val finalQualifier = foundEntry?.finalQualifierResolver ?: qualifier
        val baseDp = dpToUse.value
        return baseDp.toDynamicPerimeterPx(finalQualifier, foundEntry?.inverter ?: Inverter.DEFAULT, ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
