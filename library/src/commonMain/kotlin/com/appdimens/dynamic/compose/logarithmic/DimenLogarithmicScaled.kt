...
/**
 * EN Starts the build chain for the custom dimension DimenLogarithmicScaled from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenLogarithmicScaled a partir de um Dp base.
 */
@Composable
fun Dp.logarithmicDp(): DimenLogarithmicScaled = DimenLogarithmicScaled(this@logarithmicDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenLogarithmicScaled from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenLogarithmicScaled a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.logarithmicDp(): DimenLogarithmicScaled = this.toFloat().dp.logarithmicDp()
...
class DimenLogarithmicScaled private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenLogarithmicScaled {
        return DimenLogarithmicScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenLogarithmicScaled {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
