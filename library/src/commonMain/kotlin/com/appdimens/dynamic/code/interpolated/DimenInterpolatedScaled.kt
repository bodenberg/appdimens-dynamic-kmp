...
/**
 * EN Starts the build chain for the custom dimension DimenInterpolatedScaled from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenInterpolatedScaled a partir de um Float (Dp) base.
 */
fun Float.interpolatedScaledDp(): DimenInterpolatedScaled = DimenInterpolatedScaled(this)

/**
 * EN Starts the build chain for the custom dimension DimenInterpolatedScaled from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenInterpolatedScaled a partir de um Int (Dp) base.
 */
fun Number.interpolatedScaledDp(): DimenInterpolatedScaled = this.toFloat().interpolatedScaledDp()
...
class DimenInterpolatedScaled private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenInterpolatedScaled {
        return DimenInterpolatedScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenInterpolatedScaled = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenInterpolatedScaled = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenInterpolatedScaled = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenInterpolatedScaled = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
