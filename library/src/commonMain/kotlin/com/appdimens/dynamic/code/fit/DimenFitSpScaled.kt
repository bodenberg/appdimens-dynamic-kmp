...
class FitSp private constructor(
    ...
    /**
     * EN Resolves ftssp, fthsp, and ftwsp in one pass (single [UiModeType.fromConfiguration] read).
     * PT Resolve ftssp, fthsp e ftwsp numa só passagem.
     */
    ...
    private fun resolvePxInternal(
        ...
        return valueToUse.toDynamicFitSpPx(
            ctx,
            finalQualifier,
            finalFontScale,
    ...
    fun ftssp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.SMALL_WIDTH)
    fun fthsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.HEIGHT)
    fun ftwsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.WIDTH)
