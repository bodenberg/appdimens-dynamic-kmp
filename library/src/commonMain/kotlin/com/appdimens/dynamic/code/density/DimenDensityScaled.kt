...
/**
 * EN Starts the build chain for the custom dimension DimenDensityScaled from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDensityScaled a partir de um Float (Dp) base.
 */
fun Float.densityScaledDp(): DimenDensityScaled = DimenDensityScaled(this)

/**
 * EN Starts the build chain for the custom dimension DimenDensityScaled from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDensityScaled a partir de um Int (Dp) base.
 */
fun Number.densityScaledDp(): DimenDensityScaled = this.toFloat().densityScaledDp()
...
class DimenDensityScaled private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenDensityScaled {
        return DimenDensityScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDensityScaled = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDensityScaled = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDensityScaled = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDensityScaled = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    private fun resolveDpInternal(
        ...
        return dpToUse.toDynamicDensityDp(
            ctx,
            finalQualifier,
