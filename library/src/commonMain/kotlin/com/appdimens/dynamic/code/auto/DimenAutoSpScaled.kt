...
class AutoSp private constructor(
    ...
    /**
     * EN Resolves assp, ahsp, and awsp in one pass (single [UiModeType.fromConfiguration] read).
    ...
    private fun resolvePxInternal(
        ...
        return valueToUse.toDynamicAutoSpPx(
            ctx,
            finalQualifier,
            finalFontScale,
    ...
    fun assp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.SMALL_WIDTH)
    fun ahsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.HEIGHT)
    fun awsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.WIDTH)
