...
/**
 * EN Starts the build chain for the custom dimension DimenPercentScaled from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPercentScaled a partir de um Float (Dp) base.
 */
fun Float.percentScaledDp(): DimenPercentScaled = DimenPercentScaled(this)

/**
 * EN Starts the build chain for the custom dimension DimenPercentScaled from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPercentScaled a partir de um Int (Dp) base.
 */
fun Number.percentScaledDp(): DimenPercentScaled = this.toFloat().percentScaledDp()
...
class DimenPercentScaled private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenPercentScaled {
        return DimenPercentScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPercentScaled = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPercentScaled = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPercentScaled = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPercentScaled = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
