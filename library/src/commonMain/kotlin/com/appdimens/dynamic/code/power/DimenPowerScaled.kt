...
/**
 * EN Starts the build chain for the custom dimension DimenPowerScaled from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPowerScaled a partir de um Float (Dp) base.
 */
fun Float.powerScaledDp(): DimenPowerScaled = DimenPowerScaled(this)

/**
 * EN Starts the build chain for the custom dimension DimenPowerScaled from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPowerScaled a partir de um Int (Dp) base.
 */
fun Number.powerScaledDp(): DimenPowerScaled = this.toFloat().powerScaledDp()
...
class DimenPowerScaled private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenPowerScaled {
        return DimenPowerScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPowerScaled = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPowerScaled = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPowerScaled = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPowerScaled = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
