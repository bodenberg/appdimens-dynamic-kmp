...
class PowerSp private constructor(
    ...
    /**
     * EN Resolves pwssp, pwhsp, and pwwsp in one pass (single [UiModeType.fromConfiguration] read).
     * PT Resolve pwssp, pwhsp e pwwsp numa só passagem.
     */
    ...
    private fun resolvePxInternal(
        ...
        return valueToUse.toDynamicPowerSpPx(
            ctx,
            finalQualifier,
            finalFontScale,
    ...
    fun pwssp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.SMALL_WIDTH)
    fun pwhsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.HEIGHT)
    fun pwwsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.WIDTH)
