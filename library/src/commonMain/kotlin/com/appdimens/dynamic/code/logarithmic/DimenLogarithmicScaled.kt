 ...
 * Used by the DimenLogarithmicScaled class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenLogarithmicScaled para definir valores específicos para condições de tela.
 */
...
// EN Methods for creating the DimenLogarithmicScaled class.
// PT Métodos de criação da classe DimenLogarithmicScaled.

/**
 * EN Starts the build chain for the custom dimension DimenLogarithmicScaled from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenLogarithmicScaled a partir de um Float (Dp) base.
 */
fun Float.logarithmicScaledDp(): DimenLogarithmicScaled = DimenLogarithmicScaled(this)

/**
 * EN Starts the build chain for the custom dimension DimenLogarithmicScaled from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenLogarithmicScaled a partir de um Int (Dp) base.
 */
fun Number.logarithmicScaledDp(): DimenLogarithmicScaled = this.toFloat().logarithmicScaledDp()
 ...
 */
class DimenLogarithmicScaled private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenLogarithmicScaled {
        return DimenLogarithmicScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
     ...
     */
    @JvmOverloads
    fun ignoreMultiWindows(ignore: Boolean = true): DimenLogarithmicScaled {
        return DimenLogarithmicScaled(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenLogarithmicScaled = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenLogarithmicScaled = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenLogarithmicScaled = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenLogarithmicScaled = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    private fun resolveDpInternal(
        ...
        return dpToUse.toDynamicLogarithmicDp(
            ctx,
            finalQualifier,
