...
/**
 * EN Starts the build chain for the custom dimension DimenDiagonalScaled from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDiagonalScaled a partir de um Float (Dp) base.
 */
fun Float.diagonalScaledDp(): DimenDiagonalScaled = DimenDiagonalScaled(this)

/**
 * EN Starts the build chain for the custom dimension DimenDiagonalScaled from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDiagonalScaled a partir de um Int (Dp) base.
 */
fun Number.diagonalScaledDp(): DimenDiagonalScaled = this.toFloat().diagonalScaledDp()
...
class DimenDiagonalScaled private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenDiagonalScaled {
        return DimenDiagonalScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDiagonalScaled = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDiagonalScaled = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDiagonalScaled = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDiagonalScaled = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    private fun resolveDpInternal(
        ...
        return dpToUse.toDynamicDiagonalDp(
            ctx,
            finalQualifier,
