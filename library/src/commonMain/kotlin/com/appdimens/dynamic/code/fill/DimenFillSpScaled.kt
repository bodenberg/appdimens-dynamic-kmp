...
class FillSp private constructor(
    ...
    /**
     * EN Resolves flssp, flhsp, and flwsp in one pass (single [UiModeType.fromConfiguration] read).
     * PT Resolve flssp, flhsp e flwsp numa só passagem.
     */
    ...
    private fun resolvePxInternal(
        ...
        return valueToUse.toDynamicFillSpPx(
            ctx,
            finalQualifier,
            finalFontScale,
    ...
    fun flssp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.SMALL_WIDTH)
    fun flhsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.HEIGHT)
    fun flwsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.WIDTH)
