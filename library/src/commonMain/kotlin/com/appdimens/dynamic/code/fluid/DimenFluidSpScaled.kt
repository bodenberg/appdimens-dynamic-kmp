...
class FluidSp private constructor(
    ...
    /**
     * EN Resolves fssp, fhsp, and fwsp in one pass (single [UiModeType.fromConfiguration] read).
     * PT Resolve fssp, fhsp e fwsp numa só passagem.
     */
    ...
    private fun resolvePxInternal(
        ...
        return valueToUse.toDynamicFluidSpPx(
            ctx,
            finalQualifier,
            finalFontScale,
    ...
    fun fssp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.SMALL_WIDTH)
    fun fhsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.HEIGHT)
    fun fwsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.WIDTH)
