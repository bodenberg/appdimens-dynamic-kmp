...
class PercentSp private constructor(
    ...
    /**
     * EN Resolves pssp, phsp, and pwsp in one pass (single [UiModeType.fromConfiguration] read).
    ...
    private fun resolvePxInternal(
        ...
        return valueToUse.toDynamicPercentSpPx(
            ctx,
            finalQualifier,
            finalFontScale,
    ...
    fun pssp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.SMALL_WIDTH)
    fun phsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.HEIGHT)
    fun pwsp(ctx: DimenCallContext): Float = resolvePx(ctx, DpQualifier.WIDTH)
