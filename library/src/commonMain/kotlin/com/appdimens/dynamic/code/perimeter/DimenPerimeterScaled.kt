...
/**
 * EN Starts the build chain for the custom dimension DimenPerimeterScaled from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPerimeterScaled a partir de um Float (Dp) base.
 */
fun Float.perimeterScaledDp(): DimenPerimeterScaled = DimenPerimeterScaled(this)

/**
 * EN Starts the build chain for the custom dimension DimenPerimeterScaled from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPerimeterScaled a partir de um Int (Dp) base.
 */
fun Number.perimeterScaledDp(): DimenPerimeterScaled = this.toFloat().perimeterScaledDp()
...
class DimenPerimeterScaled private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenPerimeterScaled {
        return DimenPerimeterScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPerimeterScaled = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPerimeterScaled = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPerimeterScaled = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPerimeterScaled = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    private fun resolveDpInternal(
        ...
        return dpToUse.toDynamicPerimeterDp(
            ctx,
            finalQualifier,
