 ...
 * Used by the DimenAutoScaled class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenAutoScaled para definir valores específicos para condições de tela.
 */
...
// EN Methods for creating the DimenAutoScaled class.
// PT Métodos de criação da classe DimenAutoScaled.

/**
 * EN Starts the build chain for the custom dimension DimenAutoScaled from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenAutoScaled a partir de um Float (Dp) base.
 */
fun Float.autoScaledDp(): DimenAutoScaled = DimenAutoScaled(this)

/**
 * EN Starts the build chain for the custom dimension DimenAutoScaled from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenAutoScaled a partir de um Int (Dp) base.
 */
fun Number.autoScaledDp(): DimenAutoScaled = this.toFloat().autoScaledDp()
 ...
 */
class DimenAutoScaled private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenAutoScaled {
        return DimenAutoScaled(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
     ...
     */
    @JvmOverloads
    fun ignoreMultiWindows(ignore: Boolean = true): DimenAutoScaled {
        return DimenAutoScaled(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenAutoScaled = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenAutoScaled = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenAutoScaled = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenAutoScaled = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
